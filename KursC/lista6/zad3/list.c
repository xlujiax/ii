...
void print_string(struct List* lst)
{
  printf("%s", lst->data);
}
...
struct List* lReadLines()
{
  struct List* lines = lEmpty();
 
  int sz; // unused
  
  while(1)
    {
      lines = lConcat(lines, 0);
      if(-1 == getline(&lines->data, &sz, stdin))
        break;
    }
  return lines;
}
